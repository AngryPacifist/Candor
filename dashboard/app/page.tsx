...
import { fmtPts, fmtUnits, humanMarket, solscanTx, timeAgo, truncSig } from "../lib/format";
  ...
  return (
    <>
      ...
      <section className="grid grid-cols-[minmax(0,1fr)] overflow-hidden rounded-lg border border-border md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
        <div className="bg-panel px-4 py-5 md:px-7 md:py-6">
        ...
        <div className="flex flex-col gap-3 border-t border-border bg-background px-4 py-5 md:border-t-0 md:border-l md:px-7 md:py-6">
          <p className="text-[11px] font-medium tracking-[0.08em] text-muted uppercase">
      ...
      <div className="grid grid-cols-2 gap-3.5 lg:grid-cols-4">
        ...
        {o.nextFixture ? (
          <StatTile
            ...
            sub={`${o.nextFixture.competition} · ${new Date(o.nextFixture.start_time).toISOString().slice(5, 16).replace("T", " · ")} UTC · armed`}
            tone="accent"
          />
        ) : (
      ...
      <div className="grid grid-cols-[minmax(0,1fr)] items-start gap-6 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
        ...
        >
          {signals.length === 0 ? (
            ...
            <div>
              {signals.map((s) => (
                <div key={s.id} className="grid grid-cols-[minmax(0,1fr)] gap-1 border-t border-border py-3 first:border-t-0 first:pt-0 last:pb-0 md:grid-cols-[84px_minmax(0,1fr)] md:gap-4">
        ...
        <Panel title="Coverage" action={<span className="text-[11.5px] text-faint">auto-discovered</span>}>
          {o.upcoming.length === 0 ? (
            ...
            <div className="grid gap-3">
              {o.upcoming.map((f) => (
                <div key={f.fixture_id} className="flex items-baseline justify-between gap-3">
                  <span className="text-[13px]">
                    {f.participant1 || "TBD"} v {f.participant2 || "TBD"}
                    <span className="ml-2 text-[11px] text-faint">{f.competition}</span>
                  </span>
                  <span className="font-mono text-xs whitespace-nowrap text-muted tabular-nums">
                    {new Date(f.start_time).toISOString().slice(5, 16).replace("T", " · ")}
                  </span>
                </div>
              ))}
            </div>
